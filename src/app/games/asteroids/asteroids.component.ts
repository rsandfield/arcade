...
import { clamp, random } from '../../utility/utilities';
import { ASTEROID_SIZE_MIN, ASTEROID_SPEED_MAX, ASTEROID_SPEED_MIN, IFRAMES, POINTS, SCREEN_SIZE, STARTING_LIVES, STATE, UFOSTATS } from './constants';
import { Asteroid, Projectile, Ship, UFOSTATE, UFO } from './gameObject';
...
export class AsteroidsComponent implements OnInit {
  ...
  getCanvasContext(canvas: ElementRef<HTMLCanvasElement>) {
    ...
    context.canvas.width = SCREEN_SIZE;
    context.canvas.height = SCREEN_SIZE;
    return context;
  }
  ...
  generateAsteroid(size: number) {
    if(!this.canvas) throw new Error("Game canvas isn't");
